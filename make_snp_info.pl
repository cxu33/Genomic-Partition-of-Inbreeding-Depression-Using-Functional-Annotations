...
if($snp_pos_file =~ /bim$/) {
	...
	$pos_file_pos_col = 3;
	$pos_file_sid_col = 1;
} elsif($snp_pos_file =~ /pvar$/) {
	$pos_file_chr_col = 0;
	$pos_file_pos_col = 1;
	$pos_file_sid_col = 2;
} else {
	die "The 1st input file must be bim or pvar.\n";
}
...
while(<IN>)
{
	...
	chomp;
	my @c = split /\s+/;
	push @snp_arr, $c[$pos_file_sid_col];
	push @{$snp{$c[$pos_file_chr_col]}},[@c[$pos_file_pos_col, $pos_file_sid_col]];
}
close IN;
